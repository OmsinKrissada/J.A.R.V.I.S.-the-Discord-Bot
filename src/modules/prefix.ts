...
export default new Command({
	...
	examples: ["prefix set {prefix}", "prefix clear"],
	...
	exec(message, prefix, args, sourceID) {
		if (args[0] === "clear") {
			if (message.guild === null) {
				...
				message.channel.send(new MessageEmbed().setTitle("Prefix Cleared").setDescription(`Prefix has cleared`).setColor(Helper.GREEN));
			} else {
				message.channel.send(new MessageEmbed().setTitle("Error").setDescription(`Blank prefix is only allowed in DM channels.`).setColor(Helper.RED));
			}
		} else if (args[0] === "set") {
