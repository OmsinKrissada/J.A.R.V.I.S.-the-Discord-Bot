...
bot.on('voiceStateUpdate', async (oldvs, newvs) => {
	const condition = newvs.channel && (await settings.get(newvs.guild.id, ['enforceUserLimit'])).enforceUserLimit && newvs.channel.userLimit > 0 && newvs.channel.members.size > newvs.channel.userLimit;
	if (condition) {
